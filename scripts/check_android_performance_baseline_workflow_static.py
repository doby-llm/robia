...
def main() -> None:
    ...
    require(capture_script, 'adb shell am start -W -n "$ACTIVITY"', "validated activity launch command")
    forbid(capture_script, "adb shell am start -W -n com.gusanitolabs.robia/.MainActivity", "literal unvalidated activity launch")
    require(capture_script, 'grep -Eq "LaunchState: UNKNOWN|Status: timeout|Error:|Exception" "$LAUNCH_OUTPUT"', "launch output timeout/error rejection")
    require(capture_script, "READINESS_DEADLINE=$(( $(date +%s) + 30 ))", "bounded launch readiness deadline")
